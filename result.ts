...
export interface Match<T, E, U, V> {
  ok: (val: T) => V;
  err: (val: E) => U;
}
...
export interface Result<T, E> {
  type: symbol;
  isOk(): this is ResOk<T>;
  isErr(): this is ResErr<never, E>;
  ok(): Option<T>;
  err(): Option<E>;
  unwrap(): T | never;
  unwrapOr(optb: T): T;
  ...
  unwrapErr(): E | never;
  match<U, V>(fn: Match<T, E, U, V>): V | U;
...
export interface ResOk<T, E = never> extends Result<T, E> {
  ...
  unwrapOrElse(fn: (err: E) => T): T;
  unwrapErr(): never;
  match<U, V>(fn: Match<T, never, U, V>): V;
...
export interface ResErr<T, E> extends Result<T, E> {
  ...
  unwrapOrElse(fn: (err: E) => T): T;
  unwrapErr(): E;
  match<U, V>(fn: Match<never, E, U, V>): U;
...
export function Ok<T, E = never>(val: T): ResOk<T, E> {
  return {
    ...
    match<U, V>(matchObject: Match<T, never, U, V>): V {
...
export function Err<T, E>(err: E): ResErr<T, E> {
  return {
    ...
    match<U>(matchObject: Match<never, E, U, never>): U {
